...
class CheckedAccessFileContext:
    def __init__(self, file, warn_only=False):
        self.file = file
        self.spanset = SpanSet()
    ...
    def __check_access(self, position, s=''):
        if self.spanset.inside(position):
            if self.warn_only:
                print(f'{s}offset already accessed at {hex(position)}')
            else:
                raise Exception(f'{s}offset already accessed at {hex(position)}')
